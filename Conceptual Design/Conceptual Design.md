...
## **Power System**
- CirceBot shall run on a **lithium-ion deep-cycle battery** (Exact Model TBD, but an example will be a talentcell 12v 50Ah). This battery will power the entire robot, which includes the tracks and motors, the microcontroller, all sensors required, operating systems and the cable laying device.
...
- ### Potential "Off the Shelf" Solutions:
  Although there are several potential solutions available, it is evident that none of them fully meet the constraints provided by the customer. Many robots lack a spool, necessitating the trailing of the ethernet cable. While this approach is suitable for indoor environments, it may encounter challenges in outdoor settings due to the durability and abrasion resistance of the wire. Additionally, the majority of existing solutions rely on GPS navigation, which also does not align with the given constraints.
However, it is worth noting that autonomous drones and lawn mowers may possess ideas and hardware that could be adapted for this project. With further research and development, it may become apparent that utilizing a lawn mower is the most suitable option. Nevertheless, at this juncture, it remains uncertain.  
